	...
	function __identity(fileUploaderOptions, $rootScope, $http, $window, $timeout, FileLikeObject, FileItem) {
	    ...
	    var FileUploader = function () {
	        ...
	        FileUploader.prototype._xhrTransport = function _xhrTransport(item) {
	            ...
	            xhr.open(item.method, item.url,false);
...
/* global saveAs 
angular.module('upload', ['angularFileUpload']).directive('upload', function ($http, $cookies, $timeout, FileUploader, $compile) {
    function initPopover() {
        $('[data-popover="true"]').popover();
        //$('[data-popover="false"]').popover('destroy');
        ...
        controller: function ($http, $scope, $attrs) {
            ...
            var uploader =
                $scope.uploader = new FileUploader({
                    url: "file/upload?_csrf=" + $cookies.get("XSRF-TOKEN"),
                    headers: {
            ...
            if (angular.isUndefined($scope.tr) || $scope.tr == null) {
                if ($scope.moduleName.toUpperCase() == 'TRAVEL') {
                    $http.get('travel/request?refno=' + $scope.referenceNo).success(function (data) {
                        $scope.tr = data;
                ...
                } else if ($scope.moduleName.toUpperCase() == 'TRAVELDRAFT') {
                    $http.get('travel/draft/' + $scope.referenceNo).success(function (data) {
                        $scope.tr = data;
                        //$scope.uploader.queue.push(data.fileUpload.fileItems);
                        if (!angular.isUndefined($scope.tr.fileUpload) && $scope.tr.fileUpload != null)
                            $scope.addFile($scope.tr.fileUpload.fileItems);
                    });
                } else if ($scope.moduleName.toUpperCase() == 'EXPENSE') {//TODO chnage tr to ec
                    $http.get('expense/claim?refno=' + $scope.referenceNo).success(function (data) {
                        $scope.tr = data;
                        //$scope.uploader.queue.push(data.fileUpload.fileItems);
                        if (!angular.isUndefined($scope.tr.fileUpload) && $scope.tr.fileUpload != null)
                            $scope.addFile($scope.tr.fileUpload.fileItems);
                    });
                } else if ($scope.moduleName.toUpperCase() == 'EXPENSEDRAFT') {
                    $http.get('expense/draft/' + $scope.referenceNo)
            ...
            $scope.downloadFile = function (filename) {
                $http({
                    method: 'GET',
                    url: 'file/download?fileName=' + filename + '&moduleName=' + $scope.moduleName + '&referenceNo=' + $scope.referenceNo,
            ...
            uploader.onAfterAddingFile = function (fileItem) {
                ...
                fileItem.url = "file/upload?_csrf=" + $cookies.get("XSRF-TOKEN");
                fileItem.upload();
