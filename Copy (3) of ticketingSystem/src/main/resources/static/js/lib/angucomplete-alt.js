        ...
        .directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', '$templateCache', function ($q, $parse, $http, $sce, $timeout, $templateCache) {
            ...
            return {
                ...
                link: function (scope, elem, attrs, ctrl) {
                    ...
                    scope.$on('angucomplete-alt:setInput', function (event, elementId, data) {
                        if (!elementId || elementId === scope.id) {
                            scope.selectResult({ originalObject: data });
                        }
                    });
                    ...
                    function callOrAssign(value) {
                        ...
                        else if (scope.returnOriginalObject) {
                            //If the attribute return-original-object is specified then the additional wrapping over the object is removed and assigned
                            scope.selectedObject = value != null ? value.originalObject : null;
                        }
                        else {
                            scope.selectedObject = value;
                        }
                        ...
                        //If an onChange listener is to be bound to the directive then input-selected attribute should be used. It passes the selected result as part of the "data" argument
                        if (typeof scope.inputSelected === 'function') {
                            if (!scope.returnOriginalObject) {
                                scope.inputSelected({ data: value });
                            }
                            else {
                                scope.inputSelected({ data: value != null ? value.originalObject : null });
                            }
                        }
                    }
                    ...
                    function callFunctionOrIdentity(fn) {
                        return function (data) {
                            return scope[fn] ? scope[fn](data, scope.id) : data;
                        };
                    }
